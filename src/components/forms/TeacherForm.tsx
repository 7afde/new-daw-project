...
const TeacherForm = ({
  ...
  relatedData?: { projects: { id: number; title: string }[] };
}) => {
  const defaultProjects =
    data?.projects?.map((project: any) => project.id.toString()) || [];
  ...
  const {
    ...
    defaultValues: {
      // Set all default values from data if they exist
      ...
      sex: data?.sex || "MALE",
      specialization: data?.specialization || "GL",
  ...
  const { projects = [] } = relatedData || { projects: [] };
      ...
      <div className="flex justify-between flex-wrap gap-4">
        ...
        <div className="flex flex-col gap-2 w-full md:w-1/4">
          ...
          <select
            ...
            defaultValue={data?.specialization || "GL"}
          >
